    ...
    if (!empty( $title_link) ) { $title_page = get_post($title_link); }
		...
		?>
			...
			<ul class="social-networks">
					...
					<?php foreach(array("Twitter", "Facebook", "Flickr", "YouTube") as $network) : ?>
			    <?php if (!empty($networks[$network])) : ?><li><a rel="external" href="<?= $networks[$network] ?>"><?php if (($display == "both") or ($display =="icons")) { ?><img src="<?php bloginfo('siteurl'); ?>/wp-content/plugins/social-networks-widget/images/<?php echo strtolower($network);?>.jpg" width="34" height="34" alt=""><?php } if (($display == "labels") or ($display == "both")) {?> <?php echo $network; ?><?php } ?></a></li><?php endif; ?>
					<?php endforeach; ?>
